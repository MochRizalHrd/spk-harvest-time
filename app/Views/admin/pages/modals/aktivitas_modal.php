<!-- Modal -->
<div class="modal fade" id="dataAktivitasModal" tabindex="-1" role="dialog" aria-labelledby="dataModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <form
            method="POST" enctype="multipart/form-data"
            id="aktivitasForm"
            class="modal-content border-0 shadow rounded-4 overflow-hidden"
            action="<?= base_url('addAktivitas') ?>">
            <?= csrf_field() ?>

            <!-- Akan diubah oleh JavaScript saat edit -->
            <input type="hidden" name="id" id="aktivitas_id">
            <input type="hidden" name="_method" id="form_method" value="POST">

            <!-- Modal Header -->
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title fw-bold d-flex align-items-center gap-2" id="dataModalLabel" onclick="openAddAktivitas()">
                    Add Skala Aktivitas Ikan / Edit Skala Aktivitas Ikan
                </h5>
...
<script>
    ...
    function openAddAktivitas() {
        ...
        form.action = "<?= base_url('addAktivitas') ?>";

        // Ganti label dan tombol
        document.getElementById('dataModalLabel').innerHTML = '<i class="material-icons">person</i> Add Skala Aktivitas Ikan';
        ...
        // Tampilkan modal
        $('#dataAktivitasModal').modal('show');
    }

    // Menampilkan Form Edit
    function openEditAktivitasModal(button) {
        ...
        const form = document.getElementById('aktivitasForm');
        form.action = "<?= base_url('editAktivitas') ?>/" + id;
        ...
        document.getElementById('dataModalLabel').innerHTML = 'Edit Skala Aktivitas Ikan';
