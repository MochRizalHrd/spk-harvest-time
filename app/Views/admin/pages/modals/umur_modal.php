<!-- Modal -->
<div class="modal fade" id="dataUmurModal" tabindex="-1" role="dialog" aria-labelledby="dataModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <form
            method="POST" enctype="multipart/form-data"
            id="umurForm"
            class="modal-content border-0 shadow rounded-4 overflow-hidden"
            action="<?= base_url('addUmur') ?>">
            <?= csrf_field() ?>

            <!-- Akan diubah oleh JavaScript saat edit -->
            <input type="hidden" name="id" id="umur_id">
            <input type="hidden" name="_method" id="form_method" value="POST">

            <!-- Modal Header -->
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title fw-bold d-flex align-items-center gap-2" id="dataModalLabel" onclick="openAddUmur()">
                    Add Skala Umur / Edit Skala Umur
                </h5>
...
<script>
    ...
    function openAddUmur() {
        // Reset Form
        const form = document.getElementById('umurForm');
        form.reset();

        // Atur action form untuk "Add"
        form.action = "<?= base_url('addUmur') ?>";

        // Ganti label dan tombol
        document.getElementById('dataModalLabel').innerHTML = '<i class="material-icons">person</i> Add Skala Umur';
        ...
        // Tampilkan modal
        $('#dataUmurModal').modal('show');
    }

    // Menampilkan Form Edit
    function openEditUmurModal(button) {
        ...
        const form = document.getElementById('umurForm');
        form.action = "<?= base_url('editUmur') ?>/" + id;
        ...
        document.getElementById('dataModalLabel').innerHTML = 'Edit Skala Umur';
